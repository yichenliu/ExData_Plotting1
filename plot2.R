...
# Read the data and the class of date as date.
dt<-read.table("household_power_consumption.txt",header=TRUE,sep=";",na.strings="?")
dt[,1]<-as.Date(dt[,1],"%d/%m/%Y")

# Create a new  data table where date are only 2007-02-01 and 2007-02-02,
...
# Call "png" graphic device and save the plot as "plot2.png".
png(file="plot2.png")

# Set plot global background colour as "transparent".
par(bg="transparent")

# Set up the chart and plot the line.
